...
checkPayload($data);
$email = checkIndex($data, "user_other_email");
$password = checkIndex($data, "password");
...
$sql .= "where users.users_email = :users_email ";
...
$update = $conn->prepare("update settings_users set users_key = :users_key, users_updated = :users_updated where users_aid = :users_aid");
$update->execute([
    "users_key" => $token,
    "users_updated" => date("Y-m-d H:i:s"),
    "users_aid" => $user["users_aid"],
]);

$user["users_key"] = $token;
sendResponse([
    "data" => [mapOtherUser($user), $token],
    "count" => 1,
    "success" => true,
